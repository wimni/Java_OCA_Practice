...
public class Aufgabenstellung
{
     ...
     * Online Shop
     *
     * In this exercise we'll create program components,
     * that can be used to run an online store.
     *
     *
     * Part 1: Warehouse
     *
     ...
     *
     * The products in the warehouse (and in the next part their stock)
     * must be stored in a variable of the type Map<String, Integer>!
     ...
     *
     * Save the stock balance of products in a variable with the Map<String, Integer> type,
     * in the same way the prices were stored. Supplement the warehouse with the following methods:
     *
     ...
     * You can get the products in the warehouse from either the Map storing the prices
     * or the one storing current stock, by using the method keySet()
     *
     * An example use case:
     */
    // Warehouse warehouse = new Warehouse();
     ...
     * Part 4: Item
     *
     * Items can be added to the shopping cart (which we'll add soon).
     * An item is a product with a quantity.
     * You for example add an item representing one bread to the cart,
     * or add an item representing 24 coffees.
     *
     ...
     * milk: 5
     *
     * NB: The toString is formatted like this: product:
     * qty — price is not included in the string representation.
     *
     *
     ...
     * ShoppingCart must have an instance variable
     * with either the Map<String, Item> type, or the List<Item> type.
     * Don't add any other instance variable to the ShoppingCart class,
     * besides the List or Map used to store the items.
     *
     * NB: If you save the items in a Map type variable,
     * you'll find its values() method to be quite useful
     * for going through all the items objects stored in it for both this part of the exercise and the next.
     *
     ...
     *
     * Implement the method public void print() for the shopping cart.
     * The method prints the Item-objects in the cart.
     * The order they are printed in is irrelevant.
     * E.g. the print of the cart in the previous example would be:
     *
     * Sample output:
     * buttermilk: 1
     * cheese: 1
     ...
     *
     * Let's change our cart so that if a product is being added that's already in the cart,
     * we don't add a new item,
     ...
     * cart price: 11
     *
     * So in the example above, we first added milk and buttermilk,
     * and they get their own Item-objects.
     * When more milk is added to to cart,
     * instead of adding new items we increase the quantity in the item representing milk.
     *
     *
     * Part 8: Store
     *
     ...
     * Our store has a warehouse that includes all our products.
     * For each 'visit' we have a shopping cart.
     * Every time the customer chooses a product it's added to their cart if it's available in the warehouse.
     * At the same time, the stock in the warehouse is reduced by one.
     *
     * Below you'll find a template for a text-based user interface for our store.
     * Create a Store class for your project and copy-paste the code below there.
     *
    ...
    // }
    /*
     * The following is a main method that stocks the stores warehouse
     * and sends John to shop in the store.
     */
    // Warehouse warehouse = new Warehouse();
    ...
    // store.shop("John");
    /*
     * The store is almost done.
     * The method public void shop(String customer)
     * has a part you need to complete, marked with comments.
     * In the marked part, add code that checks if the product
     * requested by the customer is available and has stock in the warehouse.
     * If so, reduce the products stock in the warehouse
